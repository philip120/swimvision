...
def save_video_incremental(output_video_frames, output_video_path, batch_index, fps):
    ...
    if batch_index == 0:
        ...
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        temp_out = cv2.VideoWriter(temp_video_path, fourcc, fps, (output_video_frames[0].shape[1], output_video_frames[0].shape[0]))
        for frame in output_video_frames:
        ...
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter('concatenated.mp4', fourcc, fps, (output_video_frames[0].shape[1], output_video_frames[0].shape[0]))
